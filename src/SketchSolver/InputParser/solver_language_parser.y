...
%type <expression> special_predicate
...
predicate:
	identifier operator identifier {$$ = new SL::Predicate($2, new SL::Expression($1), new SL::Expression($3));}
//	|
//	expression operator expression {$$ = new SL::Predicate($2, $1, $3);}

special_predicate:
	expression operator var_val_rule {$$ = new SL::Expression(new SL::Predicate($2, $1, new SL::Expression($3)));}
...
unit: declaration {$$ = new SL::UnitLine($1);} |
	...
	//| function_call {$$ = new SL::UnitLine(new SL::Expression($1));}
...
param : expression {$$ = new SL::Param($1);} | special_predicate {$$ = new SL::Param($1);}

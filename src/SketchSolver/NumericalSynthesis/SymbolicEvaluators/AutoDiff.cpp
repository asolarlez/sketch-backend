...
/*bool isConflict(gsl_vector* g, vector<int> influentialControls) {
	for (int i : influentialControls) {
		float v = gsl_vector_get(g, i);
		if (abs(v) > 0.001) {
			return true;
		}
	}
...
set<int> AutoDiff::getConflicts(int nid) {
	...
	for (BooleanDAG::iterator node_it = bdag.begin(); node_it != bdag.end(); ++node_it) {
		...
		if (n->type == bool_node::ASSERT) {
			...
			if (val->set) {
				if (val->getVal() < 0) {
					gsl_vector_memcpy(GradUtil::tmp, val->getGrad());
					foundFailure = true;
					break;
				}
			}
		...
		if (n->getOtype() == OutType::BOOL) {
			...
			if (ival != DEFAULT_INP) {
				if (val->set) {
					float dist = val->getVal();
					if ((ival == 1 && dist < 0) || (ival == 0 && dist > 0)) {
						gsl_vector_memcpy(GradUtil::tmp, val->getGrad());
						foundFailure = true;
						break;
					}
	...
	for (int i = 0; i < nctrls; i++) {
		cout << gsl_vector_get(GradUtil::tmp, i) << ", ";
	}
	cout << endl;
	Assert(foundFailure, "No conflict?");
	vector<int> influentialControls;
	// Collect the controls that affect the failing node
	for (int i = 0; i < nctrls; i++) {
		float g = gsl_vector_get(GradUtil::tmp, i);
		if (abs(g) > 0.001) {
			influentialControls.push_back(i);
		}
	}
	
	set<int> conflictIds;
	// Collect all boolean nodes that are influenced by the controls that affect the failing node.
	for (BooleanDAG::iterator node_it = bdag.begin(); node_it != bdag.end(); ++node_it) {
		bool_node* n = *node_it;
		if (n->type == bool_node::ASSERT) {
			ValueGrad* val = v(n->mother);
			if (val->set) {
				if (inputValues.find(n->id) != inputValues.end()) {
					if (isConflict(val->getGrad(), influentialControls)) {
						cout << n->lprint() << " " << inputValues[n->id] << endl;
						conflictIds.insert(n->id);
					}
				}
			}
		}
	...
	// Need to also add all boolean nodes that are in the path to the failed assertions
	// TODO: this is hack
	if (influentialControls.size() == 0) {
		for (BooleanDAG::iterator node_it = bdag.begin(); node_it != bdag.end(); ++node_it) {
			bool_node* n = *node_it;
			if (n->getOtype() == OutType::BOOL && n->hasFloatChild()) {
				if (inputValues.find(n->id) != inputValues.end()) {
					cout << n->lprint() << " " << inputValues[n->id] << endl;
					conflictIds.insert(n->id);
				}
	...
	conflictIds.insert(nid); // always add the recently set bit

	
	Assert(conflictIds.find(nid) != conflictIds.end(), "Conflict should contain the recently set bit");
	return conflictIds;
}*/

...
int Abc_CommandSat( Abc_Frame_t * pAbc, int argc, char ** argv )
{
    ...
    if ( Abc_NtkIsStrash(pNtk) )
    {
        RetValue = Abc_NtkMiterSat( pNtk, nConfLimit, nInsLimit, fJFront, fVerbose, NULL, NULL );
    }
    else
    {
        Abc_Ntk_t * pTemp;
        pTemp = Abc_NtkStrash( pNtk, 0, 0 );
        RetValue = Abc_NtkMiterSat( pTemp, nConfLimit, nInsLimit, fJFront, fVerbose, NULL, NULL );

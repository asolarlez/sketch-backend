...
static Abc_Ntk_t * Abc_NtkMiterFraig( Abc_Ntk_t * pNtk, int nBTLimit, int * pRetValue, int * pNumFails, int * pNumConfs, int * pNumInspects );
...
int Abc_NtkMiterProve( Abc_Ntk_t ** ppNtk, void * pPars )
{
    ...
    int nSatFails, nSatConfs, nSatInspects;
    ...
    {
        clk = clock();
        RetValue = Abc_NtkMiterSat( pNtk, pParams->nMiteringLimitLast, 0, 0, 0, NULL, NULL );
    ...
    for ( nIter = 0; nIter < pParams->nItersMax; nIter++ )
    {
        ...
        // try brute-force SAT
        clk = clock();
        nSatInspects = pParams->nTotalInspectLimit? (int)(pParams->nTotalInspectLimit - pParams->nTotalInspectsMade) : 0;
        RetValue = Abc_NtkMiterSat( pNtk, (int)(pParams->nMiteringLimitStart * pow(pParams->nMiteringLimitMulti,nIter)), nSatInspects, 0, 0, &nSatConfs, &nSatInspects );
        Abc_NtkMiterPrint( pNtk, "SAT solving", clk, pParams->fVerbose );
        if ( RetValue >= 0 )
            break;
        ...
        if ( pParams->fUseFraiging )
        {
            // try FRAIGing
            clk = clock();
            pNtk = Abc_NtkMiterFraig( pNtkTemp = pNtk, (int)(pParams->nFraigingLimitStart * pow(pParams->nFraigingLimitMulti,nIter)), &RetValue, &nSatFails, &nSatConfs, &nSatInspects );  Abc_NtkDelete( pNtkTemp );
...
//            printf( "NumFails = %d\n", nSatFails );
    ...
    if ( RetValue < 0 )
    {
        ...
        clk = clock();
        nSatInspects = pParams->nTotalInspectLimit? (int)(pParams->nTotalInspectLimit - pParams->nTotalInspectsMade) : 0;
        RetValue = Abc_NtkMiterSat( pNtk, pParams->nMiteringLimitLast, nSatInspects, 0, 0, NULL, NULL );
        Abc_NtkMiterPrint( pNtk, "SAT solving", clk, pParams->fVerbose );
    }
...
***********************************************************************/
Abc_Ntk_t * Abc_NtkMiterFraig( Abc_Ntk_t * pNtk, int nBTLimit, int * pRetValue, int * pNumFails, int * pNumConfs, int * pNumInspects )
{
    Abc_Ntk_t * pNtkNew;

...
***********************************************************************/
int Abc_NtkMiterSat( Abc_Ntk_t * pNtk, int nConfLimit, int nInsLimit, int fJFront, int fVerbose, int * pNumConfs, int * pNumInspects )
{
    solver * pSat;
    lbool   status;
    int RetValue, clk;
...
//        printf( "The problem is UNSATISFIABLE after simplification.\n" );
    ...
    if ( fVerbose )
        pSat->verbosity = 1;
    status = solver_solve( pSat, NULL, NULL, nConfLimit, nInsLimit );
    if ( status == l_Undef )
    {
